...
import { hasClass } from "rz/utils/dom";
...
export default {
  ...
  methods: {
    ...
    handleClick(event) {
      ...
      if (
        target.className === "prev-month" ||
        target.className === "next-month"
      ) {
        return;
      }
      if (target.tagName !== "TD") return;
