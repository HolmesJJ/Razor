<template>
  <transition name="rz-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      class="rz-picker-panel rz-date-range-picker rz-popper"
      :class="[
        {
    ...
    >
      <div class="rz-picker-panel__body-wrapper">
        <slot name="sidebar" class="rz-picker-panel__sidebar"></slot>
        <div class="rz-picker-panel__sidebar" v-if="shortcuts">
          <div class="rz-picker-panel__sidebar-inner">
            <button
              type="button"
              class="rz-picker-panel__shortcut"
              :class="{
                'is-active':
                  clickedShortcutText && shortcut.text === clickedShortcutText
              }"
        ...
        <div class="rz-picker-panel__body">
          <div class="rz-date-range-picker__time-header" v-if="showTime">
            <span class="rz-date-range-picker__editors-wrap">
              <span class="rz-date-range-picker__time-picker-wrap">
                <rz-input
                  size="small"
                  :disabled="rangeState.selecting"
                  ref="minInput"
                  :placeholder="t('el.datepicker.startDate')"
                  class="rz-date-range-picker__editor"
              ...
              <span
                class="rz-date-range-picker__time-picker-wrap"
            ...
            <span class="rz-icon-arrow-right"></span>
            <span class="rz-date-range-picker__editors-wrap is-right">
              <span class="rz-date-range-picker__time-picker-wrap">
                <rz-input
                  size="small"
                  :disabled="rangeState.selecting"
          ...
          <div
            class="rz-picker-panel__content rz-date-range-picker__content is-left"
          >
            <div class="rz-date-range-picker__header">
              <!-- <button
                type="button"
                @click="leftPrevYear"
                class="rz-picker-panel__icon-btn rz-icon-d-arrow-left"
              ></button>-->
              <button
                type="button"
                @click="leftPrevMonth"
                class="rz-picker-panel__icon-btn rz-icon-arrow-left"
              ></button>
              <!-- <button
                type="button"
                @click="leftNextYear"
                ...
                class="rz-picker-panel__icon-btn rz-icon-d-arrow-right"
              ></button>-->
              <button
                type="button"
                @click="leftNextMonth"
                ...
                class="rz-picker-panel__icon-btn rz-icon-arrow-right"
              ></button>
              <div>{{ leftLabel }}</div>
            </div>
            <date-table
          ...
          <div
            class="rz-picker-panel__content rz-date-range-picker__content is-right"
          >
            <div class="rz-date-range-picker__header">
              <!-- <button
                type="button"
      ...
      <div class="rz-picker-panel__footer" v-if="showTime">
        <rz-button
          v-if="defaultSetting"
          type="text"
          class="rz-picker-panel__link-btn"
          @click="handleReset"
          >{{ t("el.datepicker.reset") }}</rz-button
        >
        <rz-button
          type="info"
          class="rz-picker-panel__link-btn"
          @click="handleCancel"
          >{{ t("el.datepicker.cancel") }}</rz-button
        >
        <rz-button
          type="primary"
          class="rz-picker-panel__link-btn"
          ...
          @click="handleConfirm()"
          >{{ t("el.datepicker.confirm") }}</rz-button
        >
      </div>
    </div>
  </transition>
...
import RzButton from "pkg/button";
...
const calcDefaultValue = defaultValue => {
  if (Array.isArray(defaultValue)) {
    return [
      !defaultValue[0] ? null : new Date(defaultValue[0]), 
      !defaultValue[1] ? null : new Date(defaultValue[1])
    ];
...
export default {
  ...
  computed: {
    btnDisabled() {
      /* 注释掉,防止 选择了 左边选项后,disable 不生效 */
      // if (this.clickedShortcutText !== '') {
      //   return false;
      // }
      
      // https://jira.sensetime.com/browse/SCNOVA-5427 跟据业务需求，在不限minDate，maxDate的时候需要可以confirm
  ...
  methods: {
    resetValue({value, shortcut}) {
      const defaultValue = value;
      const defaultShortcut = shortcut;
      this.minDate = calcDefaultValue(defaultValue)[0];
      this.maxDate = calcDefaultValue(defaultValue)[1];
      this.clickedShortcutText = defaultShortcut;
    },
    ...
    handleReset() {
      this.resetValue(this.defaultSetting);
    ...
    handleShortcutClick(shortcut) {
      this.$emit('click-shortcut', shortcut.text);
      if (shortcut.onClick) {
    ...
    handleConfirm(visible = false) {
      this.$emit("click-shortcut", this.clickedShortcutText, true);
  ...
  // mounted() {
  //   console.log("mounted!!");
  //   console.log("clickedText", this.clickedShortcutText);
  // },
  components: { TimePicker, DateTable, RzInput, RzButton }
};
</script>
