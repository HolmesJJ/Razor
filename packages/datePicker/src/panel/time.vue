<template>
  <transition name="rz-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div v-show="visible" class="rz-time-panel rz-popper" :class="popperClass">
      <div
        class="rz-time-panel__content"
        :class="{ 'has-seconds': showSeconds,'has-arrow': useArrow }"
      >
        <time-spinner
          ref="spinner"
          @change="handleChange"
      ...
      <div class="rz-time-panel__footer">
        <rz-button type="text" size="small" @click="handleCancel">{{ t('el.datepicker.cancel') }}</rz-button>
        <rz-button
          type="text"
          size="small"
          color="primary"
