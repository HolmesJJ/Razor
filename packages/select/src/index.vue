<template>
  <div
    class="rz-select"
    :class="[selectSize ? 'rz-select--' + selectSize : '']"
    @click.stop="toggleMenu"
    v-clickoutside="handleClose"
  >
    <div
      class="rz-select__tags"
      v-if="multiple"
      ref="tags"
    ...
    >
      <span v-if="collapseTags && selected.length">
        <rz-tag
          ...
          disable-transitions
        >
          <span class="rz-select__tags-text">{{ selected[0].currentLabel }}</span>
        </rz-tag>
        <rz-tag
          ...
          disable-transitions
        >
          <span class="rz-select__tags-text">+ {{ selected.length - 1 }}</span>
        </rz-tag>
      </span>
      <transition-group @after-leave="resetInputHeight" v-if="!collapseTags">
        <rz-tag
          ...
          disable-transitions
        >
          <span class="rz-select__tags-text">{{ item.currentLabel }}</span>
        </rz-tag>
      </transition-group>

      <input
        type="text"
        class="rz-select__input"
    ...
    >
      ...
      <i
        slot="suffix"
        :class="['rz-select__caret', 'rz-input__icon', 'rz-icon-' + iconClass]"
        @click="handleIconClick"
      ></i>
    </rz-input>
    <transition name="rz-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="doDestroy">
      <rz-select-menu
        ref="popper"
      ...
      >
        <rz-scrollbar
          tag="ul"
          wrap-class="rz-select-dropdown__wrap"
          view-class="rz-select-dropdown__list"
          ref="scrollbar"
          :class="{ 'is-empty': !allowCreate && query && filteredOptionsCount === 0 }"
          v-show="options.length > 0 && !loading"
        >
          <rz-option :value="query" parentCreated v-if="showNewOption"></rz-option>
          <slot></slot>
        </rz-scrollbar>
        <p
          class="rz-select-dropdown__empty"
          v-if="emptyText &&
...
import RzSelectMenu from "pkg/selectDropdown";
...
import { addClass, removeClass, hasClass } from "rz/utils/dom";
import { addResizeListener, removeResizeListener } from "rz/utils/resize-event";
import scrollIntoView from "rz/utils/scroll-into-view";
import { getValueByPath } from "rz/utils/util";
...
export default {
  ...
  computed: {
    ...
    showNewOption() {
      let hasExistingOption = this.options
        .filter(option => !option.parentCreated)
    ...
    autoComplete: {
      ...
      validator(val) {
        process.env.NODE_ENV !== "production" &&
          console.warn(
            "[Razor Warn][Select]'auto-complete' property will be deprecated in next major version. please use 'autocomplete' instead."
          );
        return true;
      }
    },
  ...
  methods: {
    ...
    handleIconHide() {
      let icon = this.$el.querySelector(".rz-input__icon");
      if (icon) {
    ...
    handleIconShow() {
      let icon = this.$el.querySelector(".rz-input__icon");
      if (icon && !hasClass(icon, "rz-icon-circle-close")) {
    ...
    checkDefaultFirstOption() {
      this.hoverIndex = -1;
      // highlight the parentCreated option
      let hasCreated = false;
      for (let i = this.options.length - 1; i >= 0; i--) {
        if (this.options[i].parentCreated) {
          hasCreated = true;
          this.hoverIndex = i;
          break;
        }
