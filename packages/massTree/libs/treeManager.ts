...
class TreeManager {
  ...
  setIndeterminate(node: TreeNode, isIndeterminate: boolean) {
    const target = this.getCurrentNode(node);
    target.setIndeterminate(isIndeterminate);
  }
  ...
  setStatistics(node: TreeNode, val: Statistics) {
    ...
    originNode.setStatistics(val);

    if (this.isSearch) {
      const searchNode = this.searchNodes[node._id];
      searchNode.setStatistics(val);
    }
  }
  ...
  clearSelect() {
    if (this.isSearch) {
      this.searchTree.forEach(root => root.setSelect(false));
    } else {
      this.tree.forEach(root => root.setSelect(false));
    }
    Messenger.notify({
      type: "clear-select"
    });
