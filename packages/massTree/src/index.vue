<template>
  ...
  <div class="rz-big-data-tree__container">
    ...
    <div class="rz-big-data-tree__search-result" v-if="isSearch" ref="searchResult">
...
export default class MassTree extends Vue {
  ...
  // 获取整棵树的数据
  getTree() {
    return this._provided.treeManager.getTree();
  }
  ...
  handleSearch() {
    ...
    this.data = this._provided.treeManager.getTree();
  ...
  handleClear() {
    ...
    this.isSearch = false;
    this._provided.treeManager.setIsSearch(this.isSearch);
    this.data = this._provided.treeManager.getRoot();
    this.$forceUpdate();
  }
