...
import { getStyle, hasClass, addClass, removeClass } from 'rz/utils/dom';
...
export default {
  ...
  render(h) {
    ...
    return (
      <table class="rz-table__body" cellspacing="0" cellpadding="0" border="0">
        <colgroup>
  ...
  watch: {
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      const rows = [].filter.call(tr, row => hasClass(row, 'rz-table__row'));
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      ...
      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('rz-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!hasClass(cellChild, 'rz-tooltip')) {
        return;
      }
