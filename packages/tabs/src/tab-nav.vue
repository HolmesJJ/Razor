...
export default {
  ...
  render(h) {
    ...
    const scrollBtn = scrollable
      ? [
        <span
          class={["rz-tabs__nav-prev", scrollable.prev ? "" : "is-disabled"]}
          on-click={scrollPrev}
        >
          <i class="rz-icon-arrow-left"></i>
        </span>,
        <span
          class={["rz-tabs__nav-next", scrollable.next ? "" : "is-disabled"]}
          on-click={scrollNext}
        >
      ...
      : null;
    const hasTabBarTypes = ["bar-shadow", "dark-stretch", "plain"];
    const tabBar = ~hasTabBarTypes.indexOf(type) ? (
      <tab-bar tabs={panes} tabGap={this.tabGap}></tab-bar>
    ) : null;
    ...
    const tabs = this._l(panes, (pane, index) => {
      ...
      const labelIcon = pane.icon ? (
        <i
          class={{ [`rz-icon-${pane.icon}`]: true, "rz-tab-label-icon": true }}
        ></i>
      ) : null;
      const tabLabelContent = pane.$slots.label || pane.label;
      const tabindex = pane.active ? 0 : -1;
      const style = {
        padding: `0 ${this.tabGap}px`
      };
      if (index === 0) {
        style.paddingLeft = 0;
      }
    ...
    return (
      ...
      >
        {scrollBtn}
        <div
          class={["rz-tabs__nav-scroll", `is-${this.position}`]}
          ref="navScroll"
        >
          <div
            class={[
