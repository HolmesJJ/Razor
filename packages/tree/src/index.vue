<template>
  <div
    class="rz-tree"
    :class="{
      'rz-tree--highlight-current': highlightCurrent,
  ...
  >
    ...
    ></tree-node>
    <div class="rz-tree__empty-block" v-if="isEmpty">
      <span class="rz-tree__empty-text">{{ emptyText }}</span>
    </div>
    <div v-show="dragState.showDropIndicator" class="rz-tree__drop-indicator" ref="dropIndicator"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleKeydown(ev) {
      ...
      if (currentItem.className.indexOf("rz-tree-node") === -1) return;
