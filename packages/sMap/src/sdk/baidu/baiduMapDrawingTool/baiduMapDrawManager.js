...
(function() {
  ...
  DrawingManager.prototype.clearOverlay = function () {
    this._cachedOverlays &&
      this._cachedOverlays.forEach(o => this._map.removeOverlay(o));
  }
  ...
  DrawingManager.prototype._bindPolylineOrPolygon = function() {
    ...
    (overlay = null), (isBinded = false), (overlays = []);
    var startPointPixel = null;
    var endPointPixel = null;
    var circleOverlay = null;
    me._cachedOverlays = [];

    function isClosed(startPoint, endPoint, distance = 5) {
      if (
        Math.abs(startPoint.x - endPoint.x) < distance &&
        Math.abs(startPoint.y - endPoint.y) < distance
      ) {
        return true;
      } else {
        return false;
    ...
    var startAction = function(e) {
      ...
      if (points.length == 1) {
        ...
        me._cachedOverlays.push(overlay);
        map.addOverlay(overlay);
      } else {
        startPointPixel = this._map.pointToPixel(points[0]);
        endPointPixel = this._map.pointToPixel(e.point);

        if (
          me._drawingType == BMAP_DRAWING_POLYGON &&
          points.length >= 3 &&
          isClosed(startPointPixel, endPointPixel)
        ) {
          // 判断是不是当前点是不是跟起始点靠近
          map.removeOverlay(overlay);
          drawPoint.splice(drawPoint.length - 1, 1, drawPoint[0]);
    ...
    var mousemoveAction = function(e) {
      ...
      if (
        ...
        circleOverlay = null;
        var center = map.pointToPixel(e.point);
        // 在原来点位增加十个像素的距离画圆
        var distance = map.getDistance(
          e.point,
          map.pixelToPoint({ x: center.x + 10, y: center.y })
        );
        circleOverlay = new BMap.Circle(e.point, 0, me.circleOptions);
        circleOverlay.setRadius(distance);
        me._cachedOverlays.push(circleOverlay);
  ...
  Mask.prototype.initialize = function(map) {
    ...
    div.style.cssText =
      "position:absolute;background:url(about:blank);cursor:crosshair;width:" +
      size.width +
      "px;height:" +
      size.height +
      "px";
  ...
  Mask.prototype._bind = function() {
    ...
    baidu.on(container, "mouseleave", function(e) {
      // 离开地图时，禁用边缘移动
      me.disableEdgeMove();
    });

    baidu.on(container, "mouseover", function(e) {
      me.enableEdgeMove();
    });
