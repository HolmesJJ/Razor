// https://github.com/PygmySlowLoris/vue-ripple-directive/blob/master/src/ripple.js
/**
 * binding.value = {
 *     enabel: boolean, // 是否启用波纹效果
 *     color: string // 波纹颜色
 * }
 */
var Ripple = {
    bind: function(el, binding) {
        ...
        el.addEventListener(props.event, function(event) {
            if (binding.value && binding.value.enable === false) {
                return;
            } else {
        ...
        var zIndex = Ripple.zIndex || '1999';

        function rippler(event, el) {
            var target = el;

            // 可以从指令传入值，或者默认取元素的color - 35%
            var bgColor = binding.value.color
                ? binding.value.color
