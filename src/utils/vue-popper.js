...
export default {
  ...
  methods: {
    ...
    appendArrow(element) {
      ...
      /* 添加 定制 border 的颜色 */
      if (this.arrowBorder && this.arrowBorderColor) {
        const placementArr = ["Left", "Right", "Bottom", "Top"];
        let placement = "";
        if (this.currentPlacement) {
          placement = placementArr.find(p => {
            return this.currentPlacement.indexOf(p.toLowerCase()) >= 0;
          });
          const key = `border${placement}Color`;
          arrow.style[key] = this.arrowBorderColor;
        }
      }
